...
public class Crawler {
	...
	public void crawl() {
		...
		this.currentUriQueue = null;
		this.nextUriQueue = new LinkedBlockingQueue<String>();
		nextUriQueue.add(startUrl);

		for (int i = 0; i < searchDepth; i++) {
			this.currentUriQueue = this.nextUriQueue;
			this.nextUriQueue = new LinkedBlockingQueue<String>();

			while (!this.currentUriQueue.isEmpty()) {
				String currentURI = currentUriQueue.poll();

				// only fetch this URI if it hasn't already been fetched
				if (this.alreadyHandledURIs.add(currentURI)) {
					// TODO: FETCH document
